...
const SCENARIOS = [
  {
    ...
    question: "¿Equipo Azul ganará la final del torneo nacional de fútbol?",
    context:
      "Gran final: Equipo Azul vs Equipo Rojo. Las casas de apuestas dan 50/50. Ambos equipos tuvieron una temporada fuerte.",
    insiderInfo:
      "El arquero titular del Equipo Azul está suspendido y el suplente es novato; Rojo llega con plantel completo y táctica revisada para explotar ese hueco.",
    correctAnswer: "NO"
  },
  {
    id: "fed",
    label: "Ronda 1",
    question: "¿La Reserva Federal subirá la tasa de interés en su próxima reunión?",
    context:
...
function resolveRound(game) {
  ...
  const scenario = SCENARIOS[game.currentRound];
  const now = Date.now();
...
function currentScenario(game) {
  if (game.currentRound < 0 || game.currentRound >= SCENARIOS.length) {
    return null;
  }
  return SCENARIOS[game.currentRound];
}

function nextScenario(game) {
  const next = game.currentRound + 1;
  if (next >= SCENARIOS.length) return null;
...
function buildState(game, playerId) {
  ...
  const isGameOver =
    game.currentRound >= SCENARIOS.length - 1 && game.phase === "resolved";
